...
class RRT():
    ...
    """
    def __init__(self, N, obstacles = 1, bounds=None, start=None, goal=None, stepsize=0.1, beta=0.5):
        # PROBLEM PARAMETERS
        self.N = N # dimension of the problem
        self.bounds = bounds if bounds is not None else np.ones(N) # the bounds are assumed to be symmetric
        ...
        self.obstacles = [[np.array([0.4, 0.0, 0.5]), 0.05]]
    ...
    def get_path(self):
        """ returns a list of ndarrays (points in 3D) """
...
if __name__=='__main__':
    ...
    r = RRT(N, start=np.array([0.4,0.4 ,0.0]), goal=np.array([0.4,-0.4 ,0.0]))
